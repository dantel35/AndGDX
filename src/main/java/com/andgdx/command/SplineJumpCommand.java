...
public class SplineJumpCommand extends Command implements ICommand {
	
	public float x, y, height,duration;
	private IEntity entity;
	public IEntityModifierListener listener;
	
	public SplineJumpCommand()
	{}
	
	public SplineJumpCommand(float x, float y, float height,float duration) {
	this(x,y,height,duration, null);
	}
	
	public SplineJumpCommand(float x, float y,float height, float duration, IEntityModifierListener listener) {
		super();
		this.x = x;
		this.y = y;
		this.height = height;
	...
	public void setFree()
	{
		Pool<SplineJumpCommand> pool = Pools.get(SplineJumpCommand.class);
		pool.free(this);
	}

	public void execute(IEntity entity) {
		this.entity = entity;
		entity.registerEntityModifier(AndGDXActions.splineJumpTo(x, y, height,duration, listener));
	...
	public boolean equals(ICommand command) {
		...
		if (command instanceof SplineJumpCommand)
		{
			SplineJumpCommand cmd = (SplineJumpCommand) command;
