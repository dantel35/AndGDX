...
public final class ArrayUtils {
	...
	public static <T> Array newArray(final Class<T[]> pClass, final int pLength) {
		return new Array(pClass.getComponentType());
	}
	 ...
	 */
	public static <T> Array join(final Class<T[]> pClass, final Array... pArrays) {
		if (pArrays == null) {
			return null;
		}
		...
		for (int i = pArrays.length - 1; i >= 0; i--) {
			final Array array = pArrays[i];
			if ((array != null) && (array.size > 0)) {
				resultLength += array.size;
			}
		}
		...
		/* Determine length of result. */
		final Array result = ArrayUtils.newArray(pClass, resultLength);
		int offset = 0;
	...
	public static <T> Array filter(final T[] pArray, final Class<T[]> pClass, final IMatcher<T> pMatcher) {
		...
		final Array filteredArray = ArrayUtils.newArray(pClass, filteredArrayLength);
		int resultIndex = 0;
		for (int i = 0; i < arrayLength; i++) {
			if (pMatcher.matches(pArray[i])) {
				filteredArray.set(resultIndex,pArray[i]);
				resultIndex++;
			}
		}
