...
public class OldAStarPathFinder<T>{
        ...
        public OldAStarPathFinder(final IGrid<T> pGrid, final int pMaxSearchDepth, final boolean pAllowDiagonalMovement, 
        		final IPathFinderMap<T> pPathFinderMap, final ICostFunction<T> pCostFunction) {
                this( pGrid, pMaxSearchDepth, pAllowDiagonalMovement, pPathFinderMap, new ManhattanHeuristic<T>(), pCostFunction);
        }

        public OldAStarPathFinder(final IGrid<T>  pGrid, final int pMaxSearchDepth, final boolean pAllowDiagonalMovement, final IPathFinderMap<T> pPathFinderMap,
                ...
                this.mNodes = new Node[pGrid.getAmountOfRows()][pGrid.getAmountOfColumns()];
                final Node[][] nodes = this.mNodes;
                for(int x = pGrid.getAmountOfColumns() - 1; x >= 0; x--) {
                        for(int y = pGrid.getAmountOfRows() - 1; y >= 0; y--) {
        ...
        public PathFinderPath findPath(final int pFromTileColumn, final int pFromTileRow, final int pToTileColumn, final int pToTileRow, final T pEntity) {
        	return findPath(pFromTileColumn, pFromTileRow, pToTileColumn, pToTileRow, pEntity, Integer.MAX_VALUE);
        }
        
        public PathFinderPath findPath(final int pFromTileColumn, final int pFromTileRow, final int pToTileColumn, final int pToTileRow, final T pEntity, final int pMaxCost) {
                ...
                int currentDepth = 0;
                int whileCounter = 0;//This was used to see how many times the while loop was looping. TODO: Can be deleted.
        		...
        		/* Traceback path. */
        		final PathFinderPath path = new PathFinderPath(length);
        		int index = length - 1;
        ...
        protected boolean isTileBlocked(final T pEntity, final int pFromTileColumn, final int pFromTileRow, final int pToTileColumn, final int pToTileRow) {
            System.out.println("OldASTAR fromColumn " + pFromTileColumn + " fromRow: " + pFromTileRow + " toColumn: " + pToTileColumn + " toRow: " + pToTileRow);   
        	if((pToTileColumn < 0) || (pToTileRow < 0) || (pToTileColumn >= this.mGrid.getAmountOfColumns()) || (pToTileRow >= this.mGrid.getAmountOfRows())) {
                        return true;
